...
%The following steps describe Dijkstra's algorithm.

% \begin{enumerate}
%   \item For every vertex in the graph, set the temporary distance to infinity. The starting vertex should however be set to zero.
%   \item For every vertex in the graph, mark all vertices as unvisited. Mark the starting vertex as current. Add every vertex to a set called unvisited vertices.
%   \item Calculate temporary distances for every neighbour of the current vertex. This distance is the distance of current plus the weight of the edge connecting the neighbour.
%   \item When all neighbours have been considered, mark the current node as visited, thereby removing it from the set unvisited vertices. This visited vertex will then never be checked again.
%   \item If the destination vertex has been marked visited or the smallest temporary distance in the unvisited vertices set is infinity, stop the algorithm. The smallest temporary distance in unvisted vertices set should only be infinity if there is no connecting from starting vertex to destination vertex.
%   \item From the unvisited vertices set, select the vertex with the smallest temporary vertex as the current vertex. Repeat step 3 and onwards.
% \end{enumerate}
...
\end{algorithm}
  ...
  The only notable difference between Dijkstra's algorithm and A* is the use of a heuristic value. The steps 1-6 from \cref{subss_dij_ex} are nearly identical. See \cref{fig:astar_h_explained}.
