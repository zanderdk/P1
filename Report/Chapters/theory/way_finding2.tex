...
From the unvisited vertices set, select the vertex with the smallest temporary vertex as the current vertex. Repeat step 3 and onwards. See \cref{fig:dij_step4-5-6}.
  ...
  \sinote{omskriv scenarios asdlajsldjlasjdlasjdlasdlnasndlasndnnnnnnnnnnnnnnnn}
  ...
  When calculating the heuristic function, 4 situations can occur which affects the performance of A*.
    ...
    \item If $h(x) < g(x)$, the algorithm will guaranteed find the shortest path from source to target but at a slow running time. This is because if the heuristic value is small, it does not affect the evaluation function and A* would traverse the graph in the wrong way relative to the goal. \kanote{er den helt udenfor påvirkning?}
  ...
  \kanote{overvej at fjerne g(x) og giv navne i stedet} 
...
\end{algorithm}
  ...
  The only notable difference between Dijkstra's algorithm and A* is the use of a heuristic value. The steps 1-6 from \cref{subss_dij_ex} are nearly identical. See \cref{fig:astar_h_explained}. \kanote{Jim and Jack were nearly friends.. but what is the difference}
