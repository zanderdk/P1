...
The A* algorithm was chosen as the algoritm of choice because it always finds the shortest path, and still has a reasonable fast computing time. This algorithm also makes it easier to implement stairs and elevators, because of the heuristic values. In this section we will describe A* and Dijkstra's because A* is an extension of Dijkstra's.
 ...
 \KwResult{Finds the shortest path from start vertex to destination}
  ...
  \myalg{\algo{graph g}}{}
   ...
   \tcc{Assign every permanent and temporary entry to a high integer. In this case infinity.}
  \For{\textbf{each} v in g}
  {
    $p[v] \gets  0$\;
    $t[v] \gets \infty$\;
  }
  \tcc{Start vertex has permanent label 0}
  $p[u] \gets 0$\;
  \While{p[d] = $\infty$}
  {
    \For{\textbf{each} neighbour v of u}
    {
      ...
      \If{buf < $t[v]$ \textbf{and} $p[v] = 0$}
      {
        $t[v] \gets buf$;
      }
    } 
    $u \gets v \; for \; smallest \; t[v] \; where \; p[v] = 0$\;
    $p[u] \gets t[u]$\;
  }
  \While{p[u] != 0}
  {
  \For{\textbf{each} neighbour v of u}
      {
        \If{ $v$ is not in $k$ \textbf{and} $p[u] = p[v] + distBetween(v, u)$}{
        add $u$ to $k$\;
        $u \gets v$\;
        \textbf{Break}\;
        }
