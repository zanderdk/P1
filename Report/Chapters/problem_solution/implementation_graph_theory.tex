...
%Adding new elements to the list is relatively easy compared to sequently stored data.
%It can be done by changing where what the last element of the list is point at, to a new element.
%Removing an item can be done similarly by changing the element pointing to the item that is wanted to be removed, so the list skips a element. This is shown at fig ??. %indsæt 
...
%   \tikzset{mystyle/.style={->,relative=false,in=0,out=0,bend left=100,looseness=2}}
