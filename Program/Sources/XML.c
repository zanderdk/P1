...
void crawlFloors(Floor **floors, Vertex *vertices, int numberOfFloors,
...
void crawVertices(Vertex **vertices, int last) {
    ...
    getFloorAndtype((*vertices)[last - 1].vertexId, &(*vertices)[last - 1].floorId ,
                    &(*vertices)[last - 1].type);
}
...
void crawlEdges(Edge **edges, Vertex *vertices, int numberOfEdges,
                int numberOfvertices) {
    int i, j, k;
    long vId1, vId2;
...
void readvertices(FILE *fp, Vertex *vertices, int numberOfvertices, Edge *ep,
                  int numberOfEdges) {
    char buf[16];
    memset(buf, 0, 16);
    nextElement(buf, fp);
...
Vertex readVertex(FILE *fp, Edge *ep, int numberOfEdges,
                  EdgePointer *EdgePointerStart) {
